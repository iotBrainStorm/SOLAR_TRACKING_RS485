...
void handleModbus()
{
  ...
  if (index > 0 && millis() - lastByteTime > BUS_TIMEOUT)
  {
    ...
    uint16_t receivedCRC = buffer[index - 2] | (buffer[index - 1] << 8);
    ...
    triggerLED(ledDurationRX); // RX event
    crcErrorCount = 0;
    ...
    if (function == 0x03)
    {
      ...
      {
        uint16_t value = holdingRegisters[startReg + i];
        Serial.print("Reg ");
      ...
      uint16_t crc = modbusCRC(response, 3 + regCount * 2);
      ...
      setTransmitMode();
      delayMicroseconds(300); // slightly increased for stability
      rs485.write(response, 5 + regCount * 2);
      rs485.flush();
      triggerLED(ledDurationTX); // TX event (longer blink)
      delayMicroseconds(500);    // allow bus to settle
      setReceiveMode();
  ...
  void setup()
  {
    ...
    rs485.setTimeout(2);
    lastValidPacket = millis();
    // Recommend: add 120 Ohm resistor between RS485 A/B at both ends for signal integrity
  }
